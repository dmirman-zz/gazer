...
We need to read in the file that contains data from 1 participant. The files are much too large to store on Github. 

```{r}
pupil_path <- system.file("extdata", "Pupil_file1.xls", package = "gazer")
```

With you actual data, the function `merge_pupil_files` will take all your files from a folder path and merge them together. It will also: rename columns, make all columns lowercase, and adds a new column entitled time that places time in ms instead of tracker time.
...
Subjects and trials with a lot of missing data arising from blinks should be removed. This function will remove subjects and items that pass a specififed missing threshold (we have it set at .3, but users can change this to whatever value they would like). The percetage of subjects and trials thrown out are returned for reporting in papers.   
