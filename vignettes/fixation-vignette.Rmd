...
```{r}
# Use a file installed with the pakcage 
gaze_path <- system.file(package = "gazer", "data/FixData_v1_N15.xls")
...
Filter out error and practice trials, and focus on relevant time window. Then
group by Subject, Condition, Time bin, and Object type, and calculate mean
fixation proportion for each of those "groups". These are the
subject-by-condition time courses that would go into an analysis.

```{r}
gaze_subj <- gaze_obj %>% 
  filter(ACC == 1, Time < 3500, Condition != "practice") %>% 
  group_by(Subject, Condition, Time, Object) %>%
