...
parse_edf <- function (file_list, output.dir, type="") {
  ...
  if (type=="pupil") {
    ...
    for (sub in 1:subs) { 
     ...
     dat_samp_msg <- merge(blinks, msg, by=c("subject", "time", "trial"), all.x=TRUE)
    ...
    if (type=="vwp") { 
      ...
      for (sub in 1:subs) { 
        ...
        samp1<-select(samp, time, subject, trial, x, y,pupil)
        
        fixations <- dplyr::filter(detect.fixations(samp1), event=="fixation")
        
        fixations <- dplyr::rename(fixations, CURRENT_FIX_START="start", CURRENT_FIX_END="end", CURRENT_FIX_DURATION="dur", CURRENT_FIX_Y="y", CURRENT_FIX_X="x")
       
        fixations$subject<-subject
        
        fixations <- dplyr::select(fixations, subject, trial, CURRENT_FIX_START,CURRENT_FIX_END, CURRENT_FIX_DURATION, CURRENT_FIX_Y, CURRENT_FIX_X)
        
        setwd(output.dir) 
        subOutData <- paste(file_list[sub], "_raw_vwp.csv", sep="") # save file 
        write.table(fixations, file = subOutData, append = FALSE, sep = ",",
                    row.names = FALSE, col.names = TRUE)
