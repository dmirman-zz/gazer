...
preprocess_pupil <- function(datafile, baseline_window=NA, binsize=NA, stim_onset=NA, smoothing=NA){
  ...
  if (smoothing=="LPF") {
    pupil_data_lf<- pupil_interp %>%
      ...
      dplyr::mutate(pupil_smoothed_lf=low_pass_filter(ts = interp, samp_freq=250)) #need to run the low_pass_filter function first
    ...
    baseline <- pupil_data_lf %>%
      ...
      dplyr::summarise(baseline = median(pupil_smoothed_lf, na.rm = TRUE)) #recent paper (Mahot, 2018)suggested using median over mean for baseline correction
  ...
  rolling_mean_pupil_average<-as.data.frame(pupil_interp) %>% #must be in a data.frame
    ...
    dplyr::mutate(movingavgpup= rollmean(interp,k=5, fill=NA)) #use a 5 point moving average to smooth the data
...
#baseline_window baselinemin #getting baseline pupil dimater. This takes 500 ms preceding the oneset of the similarity judgmenets.
  baseline <- interp_pupil_avg %>%
    dplyr::filter(timebins > baseline_window[1],
                  timebins < baseline_window[2]) %>%
  dplyr::group_by(subject, trial) %>%
  dplyr::summarise(baseline = median(movingavgpup, na.rm = TRUE)) #recent paper (Mahot, 2018)suggested using median over mean for baseline correction
